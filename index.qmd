...
For `distance`, cost is **meters**; for `time`, cost is **seconds**. **Nodes settled** counts the nodes for which the shortest path from my house has been found; **PQ pops** counts heap pop operations (including stale entries). As expected, A* tends to settle fewer nodes than Dijkstra with an admissible, informative heuristic.
