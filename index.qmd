...
For `distance`, cost is **meters**; for `time` and `time+grade`, cost is **seconds**. **Nodes settled** counts the nodes for which the shortest path from my house has been found; **PQ pops** counts heap pop operations (including stale entries). As expected, A* tends to settle fewer nodes than Dijkstra with an admissible, informative heuristic.
...
Use the dropdown to emphasize the metric you want to weight most.
...
Contraction hierarchies preprocess the graph so that bidirectional search on a much larger search space still yields exact shortest paths on road networks [@geisberger2008contraction]. Nodes can be merged together to create a smaller graph that is still guaranteed to contain the shortest path to the goal node while requiring fewer nodes to be relaxed. For instance, if I want to calculate the path between Georgetown University and Johns Hopkins University, it would be imprudent to explore all the side streets between Washington, DC and Baltimore. Instead, I could precalculate a super node representing the cost of all of I-95 between DC and Baltimore. With a heuristic estimating the remaining distance to the goal node, once I reach this node it will shoot me far downstream to Baltimore. 

Mapping services use contraction hierarchies to make route computation over large distances near-instantaneous. Since roads have a hierarchical structures, it is possible to simplify the graph
