...
For `distance`, cost is **meters**; for `time` and `time+grade`, cost is **seconds**. **Nodes expanded** counts settled nodes (i.e., nodes for which the shortest path from my house has been found); **PQ pops** counts heap pop operations (including stale entries). As expected, A* tends to expand fewer nodes than Dijkstra with an admissible, informative heuristic.
